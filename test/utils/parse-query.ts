...
export default function parseQueryTests() {
  describe('parseQuery', () => {
    ...
    it('should throw BadRequest if query is not an object, null or undefined', () => {
      expect(() => parseQuery(12 as any, '_id')).to.throw(errors.BadRequest)
      expect(() => parseQuery(true as any, '_id')).to.throw(errors.BadRequest)
      expect(() => parseQuery('abc' as any, '_id')).to.throw(errors.BadRequest)
      expect(() => parseQuery([] as any, '_id')).to.throw(errors.BadRequest)
    })

    it('should throw BadRequest if $or is not an array', () => {
      expect(() => parseQuery({ $or: 12 } as any, '_id')).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $or: true } as any, '_id')).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $or: 'abc' } as any, '_id')).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $or: {} } as any, '_id')).to.throw(errors.BadRequest)
    })

    it('should throw BadRequest if $and is not an array', () => {
      expect(() => parseQuery({ $and: 12 } as any, '_id')).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $and: true } as any, '_id')).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $and: 'abc' } as any, '_id')).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $and: {} } as any, '_id')).to.throw(errors.BadRequest)
    })

    it('should throw BadRequest if $sqs is not an object, null or undefined', () => {
      expect(() => parseQuery({ $sqs: 12 } as any, '_id')).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $sqs: true } as any, '_id')).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $sqs: 'abc' } as any, '_id')).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $sqs: {} } as any, '_id')).to.throw(errors.BadRequest)
    })

    it('should return null if $sqs is null or undefined', () => {
      expect(parseQuery({ $sqs: null } as any, '_id')).to.be.null
      expect(parseQuery({ $sqs: undefined }, '_id')).to.be.null
    })

    it('should throw BadRequest if $sqs does not have (array)$fields property', () => {
      expect(() => parseQuery({ $sqs: { $query: '' } } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $sqs: { $query: '', $fields: 123 } } as any)).to.throw(
        errors.BadRequest
      )
      expect(() => parseQuery({ $sqs: { $query: '', $fields: true } } as any)).to.throw(
        errors.BadRequest
      )
      expect(() => parseQuery({ $sqs: { $query: '', $fields: {} } } as any)).to.throw(
        errors.BadRequest
      )
    })

    it('should throw BadRequest if $sqs does not have (string)$query property', () => {
      expect(() => parseQuery({ $sqs: { $fields: [] } } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $sqs: { $fields: [], $query: 123 } } as any)).to.throw(
        errors.BadRequest
      )
      expect(() => parseQuery({ $sqs: { $fields: [], $query: true } } as any)).to.throw(
        errors.BadRequest
      )
      expect(() => parseQuery({ $sqs: { $fields: [], $query: {} } } as any)).to.throw(
        errors.BadRequest
      )
    })
    ...
    it('should throw BadRequest if $sqs has non-string $operator property', () => {
      expect(() =>
        parseQuery({ $sqs: { $fields: [], $query: '', $operator: [] } } as any)
      ).to.throw(errors.BadRequest)
      expect(() =>
        parseQuery({ $sqs: { $fields: [], $query: '', $operator: 123 } } as any)
      ).to.throw(errors.BadRequest)
      expect(() =>
        parseQuery({ $sqs: { $fields: [], $query: '', $operator: true } } as any)
      ).to.throw(errors.BadRequest)
      expect(() =>
        parseQuery({ $sqs: { $fields: [], $query: '', $operator: {} } } as any)
      ).to.throw(errors.BadRequest)
    ...
    it('should throw BadRequest if $child is not an object, null or undefined', () => {
      expect(() => parseQuery({ $child: 12 } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $child: true } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $child: 'abc' } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $child: [] } as any)).to.throw(errors.BadRequest)
    })

    it('should return null if $child is null or undefined', () => {
      expect(parseQuery({ $child: null } as any, '_id')).to.be.null
      expect(parseQuery({ $child: undefined }, '_id')).to.be.null
    })

    it('should return null if $child has no criteria', () => {
      expect(parseQuery({ $child: { $type: 'hello' } } as any)).to.be.null
    })

    it('should throw BadRequest if $parent is not an object, null or undefined', () => {
      expect(() => parseQuery({ $parent: 12 } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $parent: true } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $parent: 'abc' } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $parent: [] } as any)).to.throw(errors.BadRequest)
    })

    it('should return null if $parent is null or undefined', () => {
      expect(parseQuery({ $parent: null } as any, '_id')).to.be.null
      expect(parseQuery({ $parent: undefined }, '_id')).to.be.null
    })

    it('should return null if $parent has no criteria', () => {
      expect(parseQuery({ $parent: { $type: 'hello' } } as any)).to.be.null
    })

    it('should throw BadRequest if $parent does not have (string)$type property', () => {
      expect(() => parseQuery({ $parent: {} } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $parent: { $type: 123 } } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $parent: { $type: true } } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $parent: { $type: {} } } as any)).to.throw(errors.BadRequest)
    })

    it('should throw BadRequest if $nested is not an object, null or undefined', () => {
      expect(() => parseQuery({ $nested: 12 } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $nested: true } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $nested: 'abc' } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $nested: [] } as any)).to.throw(errors.BadRequest)
    })

    it('should return null if $nested is null or undefined', () => {
      expect(parseQuery({ $nested: null } as any)).to.be.null
      expect(parseQuery({ $nested: undefined })).to.be.null
    })

    it('should throw BadRequest if $nested does not have (string)$path property', () => {
      expect(() => parseQuery({ $nested: {} } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $nested: { $path: 12 } } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $nested: { $path: true } } as any)).to.throw(errors.BadRequest)
      expect(() => parseQuery({ $nested: { $path: {} } } as any)).to.throw(errors.BadRequest)
    })

    it('should return null if $nested has no critera', () => {
      expect(parseQuery({ $nested: { $path: 'hello' } } as any)).to.be.null
    })

    it('should throw BadRequest if criteria is not a valid primitive, array or an object', () => {
      expect(() => parseQuery({ age: null } as any, '_id')).to.throw(errors.BadRequest)
      expect(() => parseQuery({ age: NaN } as any, '_id')).to.throw(errors.BadRequest)
      expect(() => parseQuery({ age: (() => {}) as any } as any, '_id')).to.throw(
        errors.BadRequest
      )
    })
    ;['$exists', '$missing'].forEach((query) => {
      it(`should throw BadRequest if ${query} values are not arrays with (string)field property`, () => {
        expect(() => parseQuery({ [query]: 'foo' } as any, '_id')).to.throw(errors.BadRequest)
        expect(() => parseQuery({ [query]: [1234] } as any, '_id')).to.throw(errors.BadRequest)
        expect(() => parseQuery({ [query]: { foo: 'bar' } } as any, '_id')).to.throw(
          errors.BadRequest
        )
        expect(() => parseQuery({ [query]: [{ foo: 'bar' }] } as any, '_id')).to.throw(
          errors.BadRequest
    ...
    it('should return all queries for $and', () => {
      ...
      const expectedResult = {
        filter: [{ term: { tags: 'javascript' } }, { terms: { age: [25, 26] } }, { term: { name: 'Doug' } }],
